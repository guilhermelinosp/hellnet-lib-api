// Package server provides a reusable HTTP server wired from the config
// package, with graceful shutdown support.
package server
...
// Server wraps an *http.Server with config-driven timeouts and graceful
// shutdown handling.
type Server struct {
	http            *http.Server
...
// New builds a Server from a Config, a logger and the application's handler.
