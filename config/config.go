// Package config provides environment-driven runtime configuration.
package config
...
// FromEnv builds configuration. An empty environment defaults to Production;
// Development explicitly enables non-release behavior.
func FromEnv(build Build) (*Config, error) {
	if err := environments.LoadDotEnv(); err != nil { return nil, err }
	env := strings.TrimSpace(environments.GetString(EnvPrefix, envFallbackPrefix, "ENVIRONMENT", ""))
	if env == "" { env = "Production" }
	release := !strings.EqualFold(env, "Development")
	c := &Config{
		...
		ShutdownTimeout: 10*time.Second, ReadTimeout: 15*time.Second, WriteTimeout: 30*time.Second,
		IdleTimeout: 120*time.Second, ReadHeaderTimeout: 10*time.Second,
		CORSAllowedOrigins: list(environments.GetString(EnvPrefix, envFallbackPrefix, "CORS_ALLOWED_ORIGINS", "")),
		BodyLimit: 1 << 20, ReleaseMode: release,
...
func (c *Config) Validate() error { port, err := strconv.Atoi(c.Port); if err != nil || port < 1 || port > 65535 { return fmt.Errorf("config: %sPORT %q is invalid", EnvPrefix, c.Port) }; if strings.TrimSpace(c.Name) == "" { return fmt.Errorf("config: %sSERVICE cannot be empty", EnvPrefix) }; if c.ShutdownTimeout <= 0 || c.ReadTimeout <= 0 || c.WriteTimeout <= 0 || c.IdleTimeout <= 0 || c.ReadHeaderTimeout <= 0 { return fmt.Errorf("config: timeouts must be positive") }; if c.BodyLimit <= 0 { return fmt.Errorf("config: %sBODY_LIMIT must be positive", EnvPrefix) }; return nil }
func (c *Config) IsProduction() bool { return c.ReleaseMode }
